...
public class NotebookServer extends WebSocketServlet implements
    ...
    try {
      ...
      switch (messagereceived.op) {
        ...
        case RUN_PARAGRAPH:
          ...
          break;
        case RUN_NOTE_PARAGRAPHS:// Added by Xiufeng
          runNoteParagraphs(conn, notebook, messagereceived);
          break;
  ...
  public List<Map<String, String>> generateNotebooksInfo(boolean needsReload, HashSet<String> userAndRoles) {
    Notebook notebook = notebook();
    ...
    for (Note note : notes) {
      ...
      if (!notebookAuthorization.isOwner(note.getId(), userAndRoles)&&
          !notebookAuthorization.isReader(note.getId(), userAndRoles)&&
  ...
  public void broadcastNoteList(HashSet<String> userAndRoles) {
    List<Map<String, String>> notesInfo = generateNotebooksInfo(false, userAndRoles);
  ...
  public void broadcastReloadedNoteList(HashSet<String> userAndRoles) {
    List<Map<String, String>> notesInfo = generateNotebooksInfo(true, userAndRoles);
    broadcastAll(new Message(OP.NOTES_INFO).put("notes", notesInfo));
  }
  ...
  void permissionError(NotebookSocket conn, String op, Set<String> current,
    ...
    conn.send(serializeMessage(new Message(OP.AUTH_INFO).put("info",
        "Insufficient privileges to " + op + " job.\n\n" +
            "Allowed to  " + allowed.toString() + "\n\n" +
            "I am  " + current.toString())));
  }
  ...
  private void createNote(NotebookSocket conn, HashSet<String> userAndRoles,
    ...
    if (message != null) {
      ...
      if (noteName == null || noteName.isEmpty()) {
        noteName = "Job " + note.getId();
      }
      note.setName(noteName);
    }

    NotebookAuthorization notebookAuthorization = notebook.getNotebookAuthorization();
    notebookAuthorization.setOwners(note.getId(), userAndRoles);
    //notebookAuthorization.setReaders(note.getId(), userAndRoles);
  ...
  private void updateParagraph(NotebookSocket conn, HashSet<String> userAndRoles,
    ...
    NotebookAuthorization notebookAuthorization = notebook.getNotebookAuthorization();
    if(!notebookAuthorization.isWriter(noteId, userAndRoles)) {
      config.put("editorHide", true); //  Added by xiufeng
      config.put("tableHide", false);
      //boolean editorHide = (boolean) ((Map)fromMessage.get("config")).get("editorHide");
      //config.put("editorHide", editorHide); //  Added by xiufeng
    }
    ...
    if (notebookAuthorization.isWriter(noteId, userAndRoles)) { // added by xiufeng
  ...
  private void runNoteParagraphs(NotebookSocket conn, Notebook notebook, Message fromMessage) throws IOException {// Added by Xiufeng Liu
    final String paragraphId = (String) fromMessage.get("id");
    if (paragraphId == null) {
      return;
    }
  ...
  public static class ParagraphListenerImpl implements ParagraphJobListener {
    ...
    public void afterStatusChange(Job job, Status before, Status after) {
      if (after == Status.ERROR) {
        note.clearAll(); //Added by Xiufeng Liu
      ...
      if (job.isTerminated()) {
        ...
        note.runNextParagraph(); // Added by Xiufeng Liu
      }
